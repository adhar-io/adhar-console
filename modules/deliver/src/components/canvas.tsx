 ...
 *
 * The Delivery Flow and the Kargo promotion pipeline are both graphs, and both
 * were previously drawn as a horizontally-scrolling row of cards: readable for
 * four nodes, unusable for twenty, and with no way to step back and see the
 * shape of the thing. This provides the chrome those views were missing —
 * dotted ground, pan, zoom, fit-to-view, full-page — so each view only has to
 * describe its own nodes and edges.
 *
 * Kept local to `modules/deliver` on purpose. `packages/shell-ui` is
 * source-aliased into all nine remotes, so anything added there is bundled nine
 * times; this is used by two views in one module and belongs here.
 */
