...
/*
 * A copy of the platform module's editor, not an import.
 *
 * Each Module-Federation remote is built and shipped independently, so a
 * cross-remote import would either fail to resolve or silently duplicate the
 * whole Monaco loader at runtime. `modules/decide` keeps its own copy for the
 * same reason. Shared UI belongs in `packages/shell-ui` — but that is
 * source-aliased into all nine remotes, so putting Monaco there would bundle
 * a multi-megabyte editor nine times whether a remote uses it or not.
 */
