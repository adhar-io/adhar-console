...
const STUB_LOGS: LogEntry[] = (() => {
  const lines: Array<Pick<LogEntry, 'level' | 'message' | 'labels'>> = [
    { level: 'info', message: 'Listening on :8080', labels: { app: 'adhar-console', pod: 'adhar-console-7f8b8d5c4-xq2ws', namespace: 'demo-console' } },
    ...
    { level: 'warn', message: 'Slow database query (523ms) SELECT * FROM invoices WHERE customer_id=?', labels: { app: 'billing-service', pod: 'billing-service-5b4c6d8e9-k7fvr', namespace: 'demo-billing' } },
    { level: 'error', message: 'panic: nil pointer dereference at customer_handler.go:142 stack=...', labels: { app: 'customer-portal', pod: 'customer-portal-d4-p9lkm', namespace: 'demo-portal' } },
    { level: 'info', message: 'http.server: GET /api/repos status=200 dur_ms=142', labels: { app: 'platform-bff', pod: 'platform-bff-78a-c12kk', namespace: 'demo-platform' } },
    { level: 'warn', message: 'rate limit hit for tenant "demo-internal" — burst=12 limit=10', labels: { app: 'platform-bff', pod: 'platform-bff-78a-c12kk', namespace: 'demo-platform' } },
    { level: 'info', message: 'webhook delivered (event=push repo=adhar/adhar-console deliveries=1)', labels: { app: 'gitea', namespace: 'gitea' } },
    { level: 'error', message: 'connection refused dialing postgres:5432 (retry 2/5)', labels: { app: 'billing-service', pod: 'billing-service-5b4c6d8e9-k7fvr', namespace: 'demo-billing' } },
    { level: 'info', message: 'login_success user=tapas method=oidc tenant=default', labels: { app: 'auth-service', pod: 'auth-service-9d7-fw2mn', namespace: 'demo-auth' } },
    { level: 'info', message: 'cache hit ratio: 0.94 (last 1m)', labels: { app: 'platform-bff', pod: 'platform-bff-78a-c12kk', namespace: 'demo-platform' } },
...
const STUB_SPANS_FOR_TRACE: Record<string, Span[]> = {
  a1b2c3d4e5f60718: [
    ...
    { spanID: 's3', parentSpanID: 's1', serviceName: 'platform-bff', operationName: 'http.request gitea.adhar.local', startTimeMs: 8, durationMs: 124, tags: { 'http.method': 'GET', 'http.url': 'https://gitea.adhar.local/api/v1/orgs/adhar/repos', 'peer.service': 'gitea', 'retry.count': '0' }, events: [{ timeMs: 0, name: 'connect.start' }, { timeMs: 4, name: 'connect.established', attributes: { 'tls.version': '1.3' } }] },
    { spanID: 's4', parentSpanID: 's3', serviceName: 'gitea', operationName: 'GET /api/v1/orgs/adhar/repos', startTimeMs: 18, durationMs: 86, tags: { 'http.method': 'GET', 'http.status_code': '200', 'db.rows': '37', 'gitea.org': 'default' }, events: [{ timeMs: 60, name: 'cache.miss', attributes: { key: 'orgs:adhar:repos' } }] },
